...
const GearAvailabilityCalendar = ({ gearId, onDateSelect, selectedPickup, selectedReturn }) => {
    ...
    const fetchBookedDates = useCallback(async () => {
        try {
            ...
            console.log('Fetching gear availability for:', gearId);
            console.log('Date range:', startOfMonth.toISOString().split('T')[0], 'to', endOfMonth.toISOString().split('T')[0]);

            const { data } = await axios.get(`/api/bookings/check-availability-gear/${gearId}`, {
                params: {
                    pickupDate: startOfMonth.toISOString().split('T')[0],
                    returnDate: endOfMonth.toISOString().split('T')[0]
                }
            });

            console.log('Gear availability response:', data);

            if (data.success) {
                setBookedDates(data.bookedDates || []);
            }
        } catch (error) {
            console.error('Error fetching gear booked dates:', error);
            setBookedDates([]);
        } finally {
            setLoading(false);
        }
    ...
    const getDateStatus = (date) => {
        ...
        if (isDateBooked(date)) {
            ...
            return status === 'pending' ? 'pending' : 'booked';
        }
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-6">
                <h3 className="text-lg font-semibold text-gray-800">Gear Availability</h3>
                                        ...
                                        {(status === 'booked' || status === 'pending') && (
                                            <div className="absolute inset-0 flex items-center justify-center">
                                                <div className={`w-1 h-1 rounded-full ${
                                                    status === 'pending' ? 'bg-orange-500' : 'bg-red-500'
                                                }`}></div>
                                            </div>
                                        )}
                                    </>
...
export default GearAvailabilityCalendar;
